...
/**
 * {@link Context} is a wrapper around the entire app state. It keeps references to the internal
 * states: {@link IUserState}, {@link IEventState}, and {@link IBookingState}. The state classes are
 * kept as interfaces, so that other classes using the context cannot depend on their implementation
 * details.
 */
public class Context implements AutoCloseable, Serializable {
  private final transient String orgName;
  private final transient String orgAddress;
  private final transient String orgEmail;
  private final transient String orgSecret;
   ...
   * @param orgSecret Secret passcode only known to organisation staff, required for registering new
   *     Staff accounts
   */
