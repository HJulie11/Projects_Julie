...
/**
 * {@link IBookingState} is an interface representing the portion of application state that contains
 * all the {@link Booking} information.
 */
public interface IBookingState {
  /**
   * @return List of all registered {@link Booking}s in the application
   */
   ...
   * @return List of {@link Booking}s for the {@link Event} corresponding to the provided event
   *     number
   */
  List<Booking> findBookingsByEventNumber(long eventNumber);

  /**
   * Create a new {@link Booking} (includes generating a new unique booking number) and add it to
   * the booking state
   *
   ...
   * the booking state
   *
   * @param booking {@link Booking} to copy
   */
