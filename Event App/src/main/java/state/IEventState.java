...
/**
 * {@link IEventState} is an interface representing the portion of application state that contains
 * all the {@link Event} information.
 */
public interface IEventState {
   ...
   * @return {@link Event} corresponding to the specified event number if there is one, and null
   *     otherwise
   */
  Event findEventByNumber(long eventNumber);

  HashMap<String, EventTag> getPossibleTags();

  EventTag createEventTag(String tagName, Set<String> values, String defaultValue);

  /**
   * Create a new {@link Event} (includes generating a new unique event number) and add it to the
   * event state
   *
   * @param title name of the new event
   * @param type type of the new event
   * @param numTickets maximum number of tickets for the new event, which are all initially
   *     available
