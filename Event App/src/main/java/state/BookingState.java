...
public class BookingState implements IBookingState, Serializable {
  ...
  /**
   * Create a new BookingState that keeps track of the next booking number it will generate
   * (starting from 1 and incrementing by 1 each time a new booking number is needed), and an empty
   * list of bookings
   */
  public BookingState() {
    nextBookingNumber = 1;
  ...
  public Booking createBooking(Consumer booker, Event event, int numTickets) {
    ...
    nextBookingNumber++;
    Booking booking = new Booking(bookingNumber, booker, event, numTickets, LocalDateTime.now());
    addBooking(booking);
    return booking;
  }
