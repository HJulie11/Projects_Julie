 ...
 *
 * <p>This class should keep track of payments made, so that when a refund is requested, it can
 * check whether the transactionAmount corresponds to a previously made payment (or if something
 * fishy may be going on). Watch out for transactions made between the same people for the same
 * amount more than once!
 *
 * <p>Hint: you may find it helpful to use an inner Transaction class, overriding its equals and
 * hashCode methods
 */
public class MockPaymentSystem implements PaymentSystem {
  ...
  public boolean processPayment(
      String buyerAccountEmail, String sellerAccountEmail, double transactionAmount) {
    printMessage(
        "Processing payment",
        buyerAccountEmail,
        sellerAccountEmail,
    ...
    Transaction transaction =
        new Transaction(buyerAccountEmail, sellerAccountEmail, transactionAmount);
  ...
  @Override
  public void close() {
    // No need to clean up any resources in the mock system. But it may be necessary
    // in a real system,
