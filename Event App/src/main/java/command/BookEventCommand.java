...
/**
 * {@link BookEventCommand} allows {@link model.Consumer Consumers} to book tickets for an {@link
 * Event}. The command applies for the currently logged-in user.
 */
public class BookEventCommand implements ICommand<Booking> {
   ...
   * @verifies.that if the ticket price is greater than 0, the payment is successful before creating
   *     the booking
   */
  @Override
  public void execute(Context context, IView view) {
    // Check user is logged in and consumer
    ...
    // Check at least one ticket is being booked
    ...
    // Check event has enough tickets left
    ...
    if (ticketPrice > 0) {
      ...
      boolean paymentSucceeded =
          context
              .getPaymentSystem()
              .processPayment(consumer.getEmail(), context.getOrgEmail(), amountToPay);
    ...
    view.displaySuccess(
        ...
        Map.of("eventNumber", eventNumber, "numTicketsRequested", numTicketsRequested));
    bookingResult = booking;
  }

  /**
   * @return A unique booking number corresponding to a {@link Booking} if successful and null
   *     otherwise
   */
  @Override
