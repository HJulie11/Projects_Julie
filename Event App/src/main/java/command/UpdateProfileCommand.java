...
public abstract class UpdateProfileCommand implements ICommand<Boolean> {
   ...
   */
  protected boolean isProfileUpdateInvalid(
      Context context, IView view, String oldPassword, String newEmail) {
    // Check user logged in
    User currentUser = context.getUserState().getCurrentUser();
    if (currentUser == null) {
      view.displayFailure("UpdateProfileCommand", LogStatus.USER_UPDATE_PROFILE_NOT_LOGGED_IN);
      return true;
    }

    // Check valid password
    if (!currentUser.checkPasswordMatch(oldPassword)) {
      view.displayFailure("UpdateProfileCommand", LogStatus.USER_UPDATE_PROFILE_WRONG_PASSWORD);
      return true;
    }
    ...
    if (context.getUserState().getAllUsers().containsKey(newEmail)
      ...
      view.displayFailure(
          ...
          Map.of("old email", currentUser.getEmail(), "new email", newEmail));
      return true;
    }
  ...
  /**
   * Common update method for profile changes that involve a change of email (used as unique user
   * identifier). The method assumes error checking has already been performed before invoking this.
   *
   * @param context object that provides access to global application state
