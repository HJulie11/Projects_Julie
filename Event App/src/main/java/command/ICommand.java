 ...
 * {@link controller.Controller} to run any command using a single common method {@link
 * controller.Controller#runCommand(ICommand)}. Every command must specify a type parameter, which
 * is the command result type.
 */
public interface ICommand<T> {
  /**
   * This method should not be called directly outside of testing. Normal usage is to create a
   * command object and execute it by passing to {@link controller.Controller#runCommand(ICommand)}
   * instead.
   *
