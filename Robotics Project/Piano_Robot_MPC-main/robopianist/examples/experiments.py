...
def run_experiment(task_class, name, output_dir):
    ...
    for episode in range(FLAGS.num_episodes):
        ...
        while not timestep.last():
            # Pass the physics object explicitly from the environment
            ...
            results["key_press_reward"].append(timestep.reward.get("key_press_reward", 0))
            results["finger_movement_reward"].append(timestep.reward.get("finger_movement_reward", 0))
            results["energy_reward"].append(timestep.reward.get("energy_reward", 0))
