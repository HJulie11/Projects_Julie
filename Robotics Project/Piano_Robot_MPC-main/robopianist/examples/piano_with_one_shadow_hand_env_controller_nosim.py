...
# import robopianist.suite.tasks.piano_with_one_shadow_hand_controller as piano_with_one_shadow_hand
...
def main(_) -> None:
    ...
    fingertip_pos_traj = {finger: [] for finger in fingers_to_plot}

    for trial in range(num_trials):
        print(f"Starting trial {trial + 1}/{num_trials}")
        env.reset()
        trajectory = env.task.get_action_trajectory(env.physics)  # MP Only (no dynamics or PD)
    ...
    for finger in fingers_to_plot:
        ...
        for i in range(len(x)):
            fingertip_pos_traj[finger].append(np.array([x[i], y[i], z[i]]))
        # fingertip_pos_traj[finger].append(np.array([x_i, y_i, z_i]) for x_i, y_i, z_i in zip(x, y, z))
    ...
    plt.savefig("results/trajectories/fingertip_trajectories_controller_predefined.png")
    plt.show()
